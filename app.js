...
const localAdditions = [
  {
    ...
    website: 'https://riseartisanal.com/', mapsQuery: 'Rise Artisanal Quezon City', access: 'Pre-order + workshops', featured: true
  },
  {
    name: 'Wonderbake', region: 'Philippines', location: 'Quezon City, Metro Manila',
    ...
    website: 'https://www.google.com/search?q=Wonderbake+Quezon+City', mapsQuery: 'Wonderbake Quezon City', access: 'Store + online search'
  },
  {
    name: "Gavino's Japanese Donuts", region: 'Philippines', location: 'Granada Street area, Quezon City, Metro Manila',
    categories: ['Bakery', 'Donuts', 'Pastries'], levels: ['Beginner', 'Growing bakery'],
    ...
    website: 'https://www.google.com/search?q=Gavino%27s+Japanese+Donuts+Quezon+City', mapsQuery: "Gavino's Donuts Quezon City", access: 'Store + delivery platforms'
  },
  {
    name: 'Kamuning Bakery Cafe', region: 'Philippines', location: '43 Judge Jimenez Street corner K-1 Street, Kamuning, Quezon City',
    categories: ['Bakery', 'Traditional bread', 'Pandesal', 'Cafe'], levels: ['Beginner', 'Growing bakery', 'Professional'],
    ...
    website: 'https://www.google.com/search?q=Kamuning+Bakery+Cafe', mapsQuery: 'Kamuning Bakery Cafe Quezon City', access: 'Bakery + cafe'
  },
  {
    name: 'Pan de Manila', region: 'Philippines', location: 'Quezon City-based bakery chain with branches across Metro Manila and the Philippines',
    categories: ['Bakery', 'Pandesal', 'Traditional bread', 'Spreads'], levels: ['Beginner', 'Growing bakery'],
    ...
    website: 'https://www.google.com/search?q=Pan+de+Manila+Quezon+City', mapsQuery: 'Pan de Manila Quezon City', access: 'Branches + delivery platforms'
  }
];
...
document.getElementById('ph-stat').textContent = suppliers.filter(supplier => supplier.region === 'Philippines').length;
...
function cardTemplate(supplier) {
  ...
  return `
    <article class="card ${supplier.featured ? 'featured' : ''}">
      <div class="card-top">
        <span class="region ${supplier.region === 'International' ? 'international' : ''}">${escapeHtml(supplier.region)}</span>
      ...
      <div class="tags">${searchableTags.map(tag => `<span class="tag">${escapeHtml(tag)}</span>`).join('')}</div>
      <div class="actions">
...
function applyFilters() {
  ...
  const filtered = suppliers.filter(supplier => {
    const haystack = [supplier.name, supplier.location, supplier.bestFor, supplier.description, supplier.access,
      ...(supplier.searchTerms || []), ...supplier.categories, ...supplier.levels].join(' ').toLowerCase();
...
[regionFilter, categoryFilter, levelFilter].forEach(control => control.addEventListener('change', applyFilters));
...
const submissionForm = document.getElementById('supplier-submission-form');
...
const itemListSchema = {
  '@context': 'https://schema.org', '@type': 'ItemList', name: "The Baker's Directory listings",
  numberOfItems: suppliers.length,
  itemListElement: suppliers.map((supplier, index) => ({
    '@type': 'ListItem', position: index + 1,
    item: { '@type': 'Organization', name: supplier.name, url: supplier.website, areaServed: supplier.region, description: supplier.description }
  }))
};
